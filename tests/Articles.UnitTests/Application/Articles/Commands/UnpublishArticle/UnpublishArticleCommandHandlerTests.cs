...
namespace Articles.UnitTests.Application.Articles.Commands.UnpublishArticle;

public class UnpublishArticleCommandHandlerTests
{
    private readonly Mock<IArticleRepository> _mockRepository;
    private readonly UnpublishArticleCommandHandler _handler;

    public UnpublishArticleCommandHandlerTests()
    {
        _mockRepository = new Mock<IArticleRepository>();
        _handler = new UnpublishArticleCommandHandler(_mockRepository.Object);
    }

    [Fact]
    public async Task Handle_ExistingPublishedArticle_ShouldUnpublishArticle()
    {
        var articleId = Guid.NewGuid();
        var command = new UnpublishArticleCommand { Id = articleId };
        ...
        existingArticle.Status.Should().Be(PublicationStatus.Draft);
